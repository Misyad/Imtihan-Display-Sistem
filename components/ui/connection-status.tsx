...
export function ConnectionStatus() {
  ...
  if (!mounted) {
    return <div className="h-9 w-32 rounded-xl border border-transparent bg-transparent" />;
  }
  ...
  return (
    <div className={cn(
      "flex items-center gap-3 px-4 py-2 rounded-xl border transition-all select-none",
      config.bg,
      config.border
    )}>
      ...
      {(connectionStatus === 'disconnected' || connectionStatus === 'error') && (
        <button
          ...
          className="ml-2 text-xs font-bold text-muted-foreground hover:text-foreground transition-colors min-h-[44px] px-3 py-2 rounded-lg bg-muted hover:bg-muted/60 cursor-pointer"
        >
          Retry
        </button>
      )}
