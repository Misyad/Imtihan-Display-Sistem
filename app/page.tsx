...
export default function DashboardPage() {
  ...
  const NavCard = ({ href, icon: Icon, title, desc, color }: any) => (
    <motion.a 
      ...
      whileTap={{ scale: 0.98 }}
      className="group relative bg-card p-8 rounded-[2.5rem] border border-border shadow-sm transition-all hover:shadow-2xl hover:shadow-emerald-500/10 overflow-hidden"
    >
      ...
      <h3 className="text-xl font-black mb-2 text-foreground uppercase tracking-tight">{title}</h3>
      <p className="text-sm text-muted-foreground font-medium mb-6 leading-relaxed">{desc}</p>
    ...
    <div className="min-h-screen bg-background p-6 md:p-12 transition-colors duration-500">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <div className="flex gap-4 p-2 bg-card rounded-3xl border border-border shadow-sm">
             <div className="px-6 py-3 text-center border-r border-border">
                <p className="text-[10px] sm:text-xs font-black text-muted-foreground uppercase mb-1">Total Lembaga</p>
                <p className="text-2xl font-black text-foreground">{totalLembaga}</p>
             </div>
             <div className="px-6 py-3 text-center">
                <p className="text-[10px] sm:text-xs font-black text-muted-foreground uppercase mb-1">Total Soal</p>
                <p className="text-2xl font-black text-foreground">{totalSoal}</p>
             </div>
          </div>
        </header>
        ...
        <section className="relative overflow-hidden p-8 md:p-12 rounded-[3rem] bg-zinc-900 text-white shadow-2xl">
           ...
           <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="space-y-4 text-center md:text-left">
                 ...
                 <p className="text-display-muted font-medium max-w-xl">
              ...
              <div className="flex flex-col items-center gap-3">
                 <div className="text-center">
                    <p className="text-[10px] sm:text-xs font-black text-display-muted uppercase tracking-widest mb-2">Progres Ujian</p>
                    <div className="text-5xl font-black text-emerald-500">{totalTerjawab} <span className="text-xl text-display-muted">/ {totalSoal}</span></div>
                 </div>
        ...
        {/* Footer Statistics */}
        <footer className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-12 border-t border-border">
           <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center">
                 <Layers className="w-5 h-5 text-muted-foreground" />
              </div>
              <div>
                 <p className="text-[10px] sm:text-xs font-black text-muted-foreground uppercase">Version</p>
                 <p className="text-sm font-bold text-foreground">v2.1 Build Production</p>
              </div>
           </div>
           <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center">
              ...
              <div>
                 <p className="text-[10px] sm:text-xs font-black text-muted-foreground uppercase">Server Status</p>
           ...
           <div className="flex items-center gap-4">
              ...
              <div>
                 <p className="text-[10px] sm:text-xs font-black text-muted-foreground uppercase">Remote Support</p>
                 <p className="text-sm font-bold text-foreground">Enabled (Local IP)</p>
              </div>
           </div>
        </footer>
      </div>
