...
export default function SettingsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Settings page boundary caught an error", { digest: error.digest }, error);
  }, [error]);
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="flex min-h-[50vh] flex-col items-center justify-center rounded-2xl border border-border/40 bg-card p-8 text-center shadow-lg">
        ...
        <h1 className="mb-2 text-xl font-bold tracking-tight text-foreground">
          Gagal Memuat Halaman Pengaturan
        </h1>
        <p className="mx-auto mb-6 max-w-sm text-sm text-muted-foreground">
          Terjadi kesalahan saat memuat konfigurasi sistem. Silakan coba muat ulang atau kembali ke halaman utama.
        </p>
        {error.digest && (
